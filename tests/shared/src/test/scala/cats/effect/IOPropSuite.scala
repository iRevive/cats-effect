...
class IOPropSuite extends BaseSuite with DisciplineSuite {
  ...
  realProp(
    "parTraverseN should never exceed the maximum bound of concurrent tasks",
    for {
  ...
  realProp(
    "parTraverseN_ should never exceed the maximum bound of concurrent tasks",
    for {
      length <- Gen.chooseNum(0, 50)
      limit <- Gen.chooseNum(1, 15, 2, 5)
    } yield length -> limit
  ...
  realProp(
    "parSequenceN should give the same result as parSequence",
    Gen.posNum[Int].flatMap(n => arbitrary[List[Int]].map(n -> _))) {
    case (n, l) =>
  ...
  realProp(
    "parReplicateAN should give the same result as replicateA",
    for {
      n <- Gen.posNum[Int]
